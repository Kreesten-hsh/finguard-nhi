...
| **ASI03** | **Identity & Privilege Abuse (NHI)** | Tentative d'utilisation des credentials pour exécuter des requêtes DDL/DML arbitraires ou accéder à d'autres services. | Élévation de privilèges machine / compromission de la base bancaire. | Jetons éphémères restreints (*scoped tokens*), principe du moindre privilège, RBAC/ABAC via OPA. |
